...
public class Converter {
    ...
    FileFilter ff = new FileFilter() {
        public boolean accept(File pathname) {
            return pathname.getName().endsWith(".email");
        }        
    };
            
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            System.err.println("Usage: Converter directory outputStem");
            return;
        }
        Converter c = new Converter(args[1]);
        c.run(args[0]);
    }
    ...
    Converter(String stem) throws Exception {
        String[] flds = stem.split("/");
        currMission = flds[flds.length-1];
        err = new PrintStream(new FileOutputStream(stem+".err"));
        sql = new PrintStream(new FileOutputStream(stem+".details"));
    }
    ...
    public void run(String dir) throws Exception {
        ...
        File[] files = dirFile.listFiles(ff);
        for (File f: files) {       
            convert(f);
        }
        report();
    ...
    public void convert(File file) throws Exception {
        ...
        currentFile = currentFile.replace(".email", "");
        String line;
    ...
    public void report() {
        System.out.printf("Known lines:     %10d\n", known);
        System.out.printf("Unknown lines:   %10d\n", unknown);
        System.out.printf("Cont/Comm lines: %10d\n", contOrComm);
        System.out.printf("Singleton lines: %10d\n", singleton);
        System.out.printf("Empty lines:     %10d\n", empty);
        System.out.printf("Error lines:     %10d\n", error);
    }

    void noteFile(File file) {
        String name    = file.getName();
        name = name.replaceAll(".email$","");
        sql.println("notices|"+currMission+"|"+fileCount+"|"+name);
        fileCount += 1;
    }
    ...
    void handleNoticeType(String val) {};
    void handleNoticeDate(String val) {};
    
    void convertLine(String input) {
        if (!lastSave) {
            savedKey = null;
            savedVal = null;                    
        }
    ...
    void normal(String key, String val) {
        writeOut(sql, key, val);
    }
    
    void error(String key, String val) {
        writeOut(err, key, val);
    }
    
    void writeOut(java.io.PrintStream out, String key, String val) {        
        out.println("detail|"+currentFile+"|"+currentLine+"|"+ key+"|"+ val);
    }
